...
U = torch.from_numpy(np.load('U_case2.npy')[:1000,:,:]).float()
Q = torch.from_numpy(np.load('Q_case2.npy')[:1000,:,:10]).float()
U_t = torch.from_numpy(np.load('U_case2.npy')[1000:,:,:]).float()
Q_t = torch.from_numpy(np.load('Q_case2.npy')[1000:,:,:10]).float()
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(loader):        # gives batch data
        ...
        rnn.eval()
        test_output, h_state_t = rnn(Q_t,None)                   # (samples, time_step, input_size)
